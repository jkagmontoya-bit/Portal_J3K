/**
 * J3K Expedientes - Controlador de UI
 * Este archivo SOLO contiene funciones de interfaz (DOM).
 * Toda la lógica de estado, persistencia y reglas de negocio
 * se consume desde window.J3K_Store (store.js).
 */

// ======================================================================
// ALIASES LOCALES (Atajos para legibilidad)
// ======================================================================
const { CONFIG, state, save, emptyStep, isStepComplete, processPct, globalPct, stepStatusText, esc, safeName, money, fmtSize } = window.J3K_Store;

let currentProcess = Object.keys(CONFIG.processes)[0];

// ======================================================================
// HELPERS DE DOM (No migran al Store)
// ======================================================================
...
// ======================================================================
// stepStatus (genera HTML, por eso queda en la UI)
// ======================================================================
...
function switchExpediente(id) {
  ...
  if (target) {
    if (target.pct === 100) {
      ...
      const tryPin = () => {
        if (window.J3K_UI) {
          window.J3K_UI.showPinModal(attempts, async (pin) => {
            try {
              window.J3K_UI.showLoading("Verificando credenciales...");
              const resp = await window.J3K_API.auth.verifyMasterPin(pin);
              ...
              attempts--;
              if (attempts > 0) {
                window.J3K_UI.showToast("PIN incorrecto. Inténtalo de nuevo.", "error");
                tryPin();
              } else {
          ...
          alert("Error: Gestor UI no cargado.");
        }
      };
  ...
  Array.from(files || []).forEach(file => {
    ...
    reader.onload = async (e) => {
      ...
      if (window.uploadToFirebaseStorage) {
        try {
          const path = 'expedientes/' + currentProcess + '/' + Date.now() + '_' + file.name;
          const url = await window.uploadToFirebaseStorage(path, finalDataUrl);
          finalDataUrl = url;
        } catch (err) {
      ...
      state.processes[currentProcess].steps[i].files.push({ name: file.name, type: file.type || "application/octet-stream", size: file.size, data: finalDataUrl, addedAt: new Date().toISOString() });
...
async function procesarFacturaConIA(file, dataUrl) {
  ...
  titleEl.innerHTML = "🤖 Analizando factura en servidor seguro... espere por favor.";
  titleEl.style.backgroundColor = "#dbeafe";
  titleEl.style.color = "#1e40af";

  try {
    let data;
    if (window.J3K_API && window.J3K_API.ia) {
      const resp = await window.J3K_API.ia.extraerFactura(dataUrl, 'factura');
      if (!resp.success) throw new Error("Error en Backend IA");
      data = {
        razon_social: resp.extraccion.ruc_emisor,
        ruc: resp.extraccion.ruc_emisor,
        numero_factura: "",
        proyecto: "",
        descripcion: resp.extraccion.descripcion || "Autocompletado seguro",
        monto: resp.extraccion.total
      };
    } else {
...
function entriesForProcess(processKey, rootPrefix = "") {
  ...
  p.steps.forEach((step, i) => {
    ...
    const meta = { paso: i + 1, nombre: step.name, documento_requerido: step.requiredDoc, obligatorio: step.required, responsable: step.responsible, control: step.controlNote, estado: stepStatusText(currentProcess, i), observaciones: Array.isArray(s.steps[i].obs) ? s.steps[i].obs : (s.steps[i].obs ? [{ text: s.steps[i].obs, date: new Date().toISOString() }] : []), archivos: (s.steps[i].files || []).map(f => f.name) };
